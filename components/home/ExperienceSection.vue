	...
	<div class="home--experience">
		...
		<v-card v-for="exp in experiences" :key="exp.company" class="mb-8">
			...
			<v-card-text>
				...
				>
					<v-card-title class="list">
						<h4 class="ellipses" :title="proj.name">
							{{proj.name}}
						</h4>
						...
						<template v-if="proj.logo">
							<v-spacer />
							<img :src="getAssetUrl(proj.logo)"
									 :height="proj.logoStyle?.height || 60"
									 :width="proj.logoStyle?.width || 100"
									 class="rounded"
									 :style="'object-fit: ' + (proj.logoStyle?.objectFit || 'contain')"
							/>
						</template>
					</v-card-title>
