...
<template>
	<v-toolbar class="home--toolbar" density="compact" :extended="width < 820">
		...
		<div v-if="width >= 820" class="btn-grp">
			<button v-ripple class="btn" @click="scrollTo.works()">
				<v-icon>mdi-file</v-icon>
				Works
			</button>
			<button v-ripple class="btn" @click="router.push({ name: 'Resume' })">
				<v-icon>mdi-account-tie</v-icon>
				Resume
			</button>
			<button v-ripple class="btn" @click="router.push({ name: 'Blogs' })">
				<v-icon>mdi-post</v-icon>
				Blogs
			</button>
			<v-btn size="small" title="Change Theme" icon @click="changeTheme()">
				<v-icon size="24">
		...
		<template v-if="width < 820" #extension>
			<div class="btn-grp px-1">
				<div v-ripple class="btn" title="Works" @click="scrollTo.works()">
					<v-icon>mdi-file</v-icon>
