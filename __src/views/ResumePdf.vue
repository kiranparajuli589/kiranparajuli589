	...
	<div class="resume-pdf">
		<div class="pdf">
			...
			<div class="p-list">
				...
				<a target="_blank" class="dev-to-link" :href="personalInfo.devto">
			...
			<div v-for="tech in technologies" :key="tech.name">
				...
				<div>
					<template v-for="(tool, index) in tech.tools">
						{{ tool.tooltip
						}}<template v-if="index !== tech.tools.length - 1"
							>,&nbsp;&nbsp;</template
						>
					</template>
				</div>
			...
			<div v-for="(work, index) in works" :key="work.title" class="mb-6">
				...
				<div class="list mb-2">
					...
					<template
						...
						:key="key"
					>
						<a target="_blank" class="text-capitalize" :href="value">
							{{ key }}
